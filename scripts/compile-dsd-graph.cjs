...
// Load actual product slugs so we only assign /product/ URLs to real product pages
let productSlugs = [];
if (fs.existsSync(PRODUCTS_FILE)) {
  try {
    const products = JSON.parse(fs.readFileSync(PRODUCTS_FILE, 'utf-8'));
    productSlugs = products.map(p => p.slug || p.catalogId || p.id).filter(Boolean);
    console.log(`  ${productSlugs.length} product page(s): ${productSlugs.join(', ')}`);
  } catch (e) { console.warn('  Could not load products.json:', e.message); }
}
...
for (const entry of catalog) {
  ...
  addNode({
    ...
    title: entry.title,
    url: productSlugs.includes(entry.id) ? `/product/${entry.id}` : writings.find(w => w.slug === entry.id || w.slug === entry.id.replace(/-song$/, '')) ? `/writings/${(writings.find(w => w.slug === entry.id || w.slug === entry.id.replace(/-song$/, ''))).slug}` : null,
    surface: {
      thumbnail: null,
